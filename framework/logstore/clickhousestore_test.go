...
func TestClickHouseEnsureExtensionTable(t *testing.T) {
	...
	opts := ClickHouseExtensionTableOptions{
		...
		SkipIndexes: []string{"INDEX idx_extension_value value TYPE bloom_filter GRANULARITY 1"},
	}
...
func TestValidateClickHouseExtensionTableOptions(t *testing.T) {
	...
	for name, mutate := range map[string]func(*ClickHouseExtensionTableOptions){
		"partition statement": func(opts *ClickHouseExtensionTableOptions) { opts.PartitionBy = "id) ENGINE = Null (" },
		"order comment":       func(opts *ClickHouseExtensionTableOptions) { opts.OrderBy = "id -- ignore" },
		"ttl statement":       func(opts *ClickHouseExtensionTableOptions) { opts.TTL = "created_at; DROP TABLE logs" },
		"index statement": func(opts *ClickHouseExtensionTableOptions) {
			opts.SkipIndexes = []string{"INDEX idx value TYPE bloom_filter GRANULARITY 1) ENGINE = Null"}
		},
	} {
		t.Run(name, func(t *testing.T) {
			opts := valid
			mutate(&opts)
			require.Error(t, validateClickHouseExtensionTableOptions(opts))
		})
	}
}
